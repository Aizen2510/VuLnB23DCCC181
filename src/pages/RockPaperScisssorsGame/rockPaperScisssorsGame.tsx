...
const rockPaperScisssorsGame: React.FC = () => {
	...
	const [gameState, setGameState] = useState<'waiting' | 'pause' | 'playing' | 'exit'>('waiting');
		...
		<Layout className='game-container'>
				...
				<Card className='game-card'>
					{gameState === 'waiting' ? (
						<Space direction='vertical' size='large' className='game-space'>
							...
							<Button type='primary' icon={<PlayCircleOutlined />} onClick={startGame}>
								Bắt Đầu Chơi
							</Button>
						</Space>
					) : (
						<Space size='large' direction='vertical' style={{ width: '100%' }}>
							{result && (
								<Alert
									...
									type={result === 'Thắng' ? 'success' : result === 'Thua' ? 'error' : 'info'}
									showIcon
							...
							<Row gutter={[16, 24]} justify='center'>
								<Col>
									<Space size='large'>
										<Button size='large' onClick={() => playGame('Kéo')} className='choice-button'>
											<img src={handScissor} alt='' />
											Kéo
										</Button>
										<Button size='large' onClick={() => playGame('Búa')} className='choice-button'>
											<img src={handRock} alt='' />
											Búa
										</Button>
										<Button size='large' onClick={() => playGame('Bao')} className='choice-button'>
							...
							<Col span={24}>
								<Card className='current-game-card'>
									<Space direction='vertical' style={{ width: '100%' }}>
										<Text strong>
											Bạn chọn: <span className='choice-text'>{playerChoice}</span>
										</Text>
										<Text strong>
											Máy chọn: <span className='choice-text'>{computerChoice}</span>
										</Text>
										<Text strong>
											Kết quả:
											<span style={{ color: getResultColor(result), marginLeft: 8 }}>{result}</span>
										</Text>
									</Space>
								</Card>
							</Col>
			...
			>
				{history.length > 0 ? (
						...
						<List
							...
							renderItem={(item, index) => (
								<List.Item className='history-item'>
									<Text>
										Ván {history.length - index}: Bạn ({item.playerChoice}) vs Máy ({item.computerChoice}) -
										<Tag color={getResultColor(item.result)} style={{ marginLeft: 8 }}>
											{item.result}
										</Tag>
									</Text>
								</List.Item>
					...
					<Paragraph className='empty-history'>Chưa có lịch sử đấu nào</Paragraph>
				)}
			</Modal>
			...
			>
				<Row gutter={[16, 16]} className='stats-container'>
					...
					<Col span={12}>
						<Statistic title='Tỷ lệ thắng' value={winRate} suffix='%' valueStyle={{ color: '#3f8600' }} />
					</Col>
					<Col span={8}>
						<Statistic title='Thắng' value={wins} valueStyle={{ color: 'green' }} />
					</Col>
					<Col span={8}>
						<Statistic title='Thua' value={losses} valueStyle={{ color: 'red' }} />
					</Col>
					<Col span={8}>
						<Statistic title='Hòa' value={draws} valueStyle={{ color: 'blue' }} />
					</Col>
					<Col span={12}>
						<Statistic
							title='Điểm hiện tại'
					...
					<Col span={12}>
						<Statistic title='Điểm trung bình mỗi trận' value={totalGames > 0 ? (points / totalGames).toFixed(1) : 0} />
					</Col>
				</Row>
			</Modal>
		</Layout>
